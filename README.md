...
在 Elements 中，点击箭头展开节点，点击标签或节点行空白编辑 HTML，点击属性编辑名称和值，点击文本或注释编辑文字。工具栏“编辑”按钮也可编辑当前选中的节点，包括通过页面取点选中的节点。
...
构建将 `elements-editor.js` 和 `vendor/eruda.js` 内嵌到 `page.js`，再内嵌到 `userscript.js`，生成完整安装文件。上游文件保留原样，构建时校验哈希并移除顶层 source map 下载引用。项目不引入运行时远程依赖，打包和测试使用 Node.js 内置能力。
