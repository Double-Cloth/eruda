...
Elements 顶部控制栏靠右的“插入”按钮可在当前选中节点的之前、内部开头、内部末尾或之后插入 DOM；按钮位于原生图标左侧，不支持子节点的位置会自动禁用。一次可插入多个元素、文本或注释，并按目标上下文保留表格、SVG、MathML、template 与 Shadow DOM 语义。插入位置已被页面更新时会提示重新选择。
