...
在 Elements 中，点击箭头展开节点，点击标签、节点行空白、属性、文本或注释只负责选中，不会直接打开编辑器。选中后点击顶部控制栏的“编辑”按钮：标签或节点行空白编辑 HTML，属性编辑名称和值，文本或注释编辑文字；通过页面取点选中的节点也可用同一按钮编辑。

编辑器支持 HTML、文本、属性模式，提供离线语法高亮及明暗主题。点击“应用修改”写回页面，“取消”或 Escape 放弃修改。

Elements 顶部控制栏靠右提供与原生工具栏协调的“编辑 / 插入”分段按钮。“插入”可在当前选中节点的之前、内部开头、内部末尾或之后插入 DOM；不支持子节点的位置会自动禁用。一次可插入多个元素、文本或注释，并按目标上下文保留表格、SVG、MathML、template 与 Shadow DOM 语义。插入位置已被页面更新时会提示重新选择。
