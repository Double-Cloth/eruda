...
| 点击位置 | 编辑内容 |
| --- | --- |
| 元素标签、结束标签或节点行空白 | 完整节点 HTML，可修改标签、属性和子节点；支持空 div、无属性节点及嵌套节点 |
| 属性名或属性值 | 修改 / 新增 / 改名属性，也可删除属性；空值与删除分开处理 |
| 文本或注释 | 修改当前文字，保留同级节点 |
| 工具栏“编辑” | 编辑当前选中的节点，也支持通过页面取点选中的节点 |

编辑器内可以切换 HTML、文本和属性模式。点击“应用修改”立即写回页面；“取消”或 Escape 放弃本次修改。HTML 模式必须输入一个完整元素；html、head、body 结构根节点仅允许编辑属性，请选择其子节点修改内容。节点被页面移除或待编辑内容已被更新时，会提示重新选择，避免覆盖过期内容。
...
属性修改保留原节点和事件监听；HTML 替换会创建新节点，元素的文本模式会替换全部子节点。修改只作用于当前页面，刷新后恢复，页面框架的重新渲染也可能覆盖修改。
